...
mdp::vector<int> read_from_file(const char* filename) {
    // Option 1: create and open
    //std::ifstream is;
    //is.open(filename/*, std::ios::binary*/);
    // Option 2: RAII: Resource Allocation Is Initialization
    ...
    //while (true) {
    //    int val;
    //    // int c = is.get(); // Reads one byte (fgetc)
    //    // is.unsetf(std::ios::skipws); // Disables ws skipping 
    //    is >> val; // fscanf(is, "%d", val);
    //    if (!is) {
    //        break;
    //    }
    //    v.push_back(val);

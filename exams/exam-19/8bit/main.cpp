...
Image readBMP(std::istream& is) {
	...
	std::map<uint8_t, Color> colorTable;
	...
	for (size_t i = 0; i < img.rows(); i++) {
		size_t byteToRead = img.cols();
		std::vector<uint8_t> buffer(byteToRead);
		is.read(reinterpret_cast<char*>(buffer.data()), byteToRead);
		std::vector<uint8_t> row(byteToRead * 3);

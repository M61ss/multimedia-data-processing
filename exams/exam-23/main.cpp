...
struct FileHeader {
	...
	uint32_t samplesPerChannel_ = 0;

	FileHeader() : magicNumber_(4, 0), samplesPerChannel_(0) {}
};

struct FrameHeader {
	uint8_t nChannels_ = 0;
...
private:
	...
	void dequantize(const double &sf, const std::vector<uint8_t> &qr, LMSState &lms) {
		for (const auto &q : qr) {
			...
			p >>= 13;
			int64_t s = p + r;
			s = std::clamp(s, -32768ll, 32767ll);
			data_.push_back(static_cast<int16_t>(s));
			...
			lms.history_[3] = static_cast<int16_t>(s);
		}
	}
...
public:
	...
	void decode() {
		...
		while (true) {
			...
			for (size_t s = 0; s < (frameHdr.frameSamples_ + 19) / 20; s++) {
				for (size_t c = 0; c < frameHdr.nChannels_; c++) {
					uint64_t slice = 0;
...
void saveWAV(std::ostream &os, const QOADecoder &qoa) {
	...
	os << "data";
	size_t dataSize = qoa.fileHdr_.samplesPerChannel_ * 4; // qoa.data().size() * 2;
	os.write(reinterpret_cast<const char *>(&dataSize), 4);

	size_t n = 0;
	for (size_t i = 0; i < qoa.fileHdr_.samplesPerChannel_ * 2; i += 40) {
		if (n == qoa.fileHdr_.samplesPerChannel_) {
			break;
		}
		for (size_t j = 0; j < 20; ++j) {
			...
			os.write(reinterpret_cast<const char *>(qoa.data().data() + i + j), 2);
			os.write(reinterpret_cast<const char *>(qoa.data().data() + i + j + 20), 2);
			++n;
		}
	}	

	fileSize = size_t(os.tellp()) - 8;
	os.seekp(4);
	os.write(reinterpret_cast<const char *>(&fileSize), 4);
}
